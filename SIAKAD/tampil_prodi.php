...
<table id="customers">
  <tr>
    ...
    <th width="210">Alamat Prodi</th>
    <th width="350">Telepon</th>
    <th width="350">Id Dosen</th>
    <th>Update</th>
    <th>Delete</th>
  </tr>
  <?php
    include 'koneksi.php';
    $datauser = mysqli_query($kon,"select * from prodi");
    while ($tampil = mysqli_fetch_array($datauser)){
    ?>
    <tr>
        <td><?php echo $tampil ['idProdi'];?></td>
        <td><?php echo $tampil ['namaProdi'];?></td>
        <td><?php echo $tampil ['alamatProdi'];?></td>
        <td><?php echo $tampil ['noTelp'];?></td>
        <td><?php echo $tampil ['idDosen'];?></td>
        <td><a href="formedit_prodi.php?idprodi=<?php echo $tampil['idProdi'];?>" onclick="return confirm('Apakah Anda Yakin?')">Edit</a></td>
        <td><a href="deleteprodi.php?idprodi=<?php echo $tampil['idProdi'];?>" onclick="return confirm('Apakah Anda Yakin?')">Delete</a></td>
    </tr>
    <?php } ?>
</table>
</body>
