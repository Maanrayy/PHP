...
<a href="INDEX.html"><button class="dropbtn">DASHBOARD</button></a>
  ...
  <a href="tampil_matkul.php">LIHAT DATA</a>
    <a href="input_matkul.php">INPUT MATKUL</a>
  </div>
...
<table id="customers">
  <tr>
    <th width="100">Id Matkul</th>
    <th width="200">Nama Matkul</th>
    <th width="210">Id Dosen</th>
    <th>Update</th>
    <th>Delete</th>
    ...
    <?php
    include 'koneksi.php';
    $datauser = mysqli_query($kon,"select * from matkul");
    while ($tampil = mysqli_fetch_array($datauser)){
    ?>
    <tr>
        <td><?php echo $tampil ['idMatkul'];?></td>
        <td><?php echo $tampil ['namaMatkul'];?></td>
        <td><?php echo $tampil ['idDosen'];?></td>
        <td><a href="formedit_matkul.php?iduser=<?php echo $tampil['idMatkul'];?>" onclick="return confirm('Apakah Anda Yakin?')">Edit</a></td>
        <td><a href="deletematkul.php?idmatkul=<?php echo $tampil['idMatkul'];?>" onclick="return confirm('Apakah Anda Yakin?')">Delete</a></td>
    </tr>
    <?php } ?>
    </table>
</div>
